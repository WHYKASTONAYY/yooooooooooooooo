                ...
                if photo_video_group_details:
                    ...
                    try:
                        for item in photo_video_group_details:
                            ...
                            # Try file_id first, but catch invalid file_id errors
                            if item['id']:
                                try:
                                    logger.debug(f"Attempting to use Telegram file_id for P{prod_id}: {item['type']}")
                                    ...
                                    file_id_used = True
                                    logger.debug(f"Successfully prepared media with file_id for P{prod_id}: {item['type']}")
                                except Exception as file_id_error:
                                    logger.warning(f"Invalid file_id {item['id']} for P{prod_id}: {file_id_error}. Falling back to local file.")
                                    input_media = None  # Reset and try file path
                            
                            # Fall back to file path if file_id failed or doesn't exist
                            if not file_id_used and item['path'] and await asyncio.to_thread(os.path.exists, item['path']):
                ...
                if animations_to_send_details:
                    ...
                    for item in animations_to_send_details:
                        ...
                        try:
                            ...
                            if item['id']:
                                try:
                                    logger.debug(f"Attempting to use Telegram file_id for animation P{prod_id}")
                                    media_to_send_ref = item['id']
                                    # Test send to validate file_id before proceeding
                                    await context.bot.send_animation(chat_id=chat_id, animation=media_to_send_ref)
                                    file_id_used = True
                                    logger.info(f"✅ Successfully sent animation with file_id for P{prod_id} user {user_id}")
                                except Exception as file_id_error:
                                    logger.warning(f"Invalid file_id {item['id']} for animation P{prod_id}: {file_id_error}. Falling back to local file.")
                                    media_to_send_ref = None
