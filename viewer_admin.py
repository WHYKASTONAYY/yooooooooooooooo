...
async def handle_viewer_view_product_media(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    try:
        for i, item in enumerate(media_items): # item is now a Row object
            ...
            try:
                if file_id:
                    # Try using file_id first, but catch invalid file_id errors
                    try:
                        ...
                        # Test if the file_id is valid by creating a temporary input media object
                        # If this succeeds, we'll use it; if it fails, we'll fall back to file path
                        
                    except Exception as file_id_error:
                        logger.warning(f"Invalid file_id {file_id} for P{product_id}: {file_id_error}. Falling back to local file.")
                        input_media = None  # Reset and fall back to file path
                        
                if not input_media and file_path and await asyncio.to_thread(os.path.exists, file_path): # Check existence before open
